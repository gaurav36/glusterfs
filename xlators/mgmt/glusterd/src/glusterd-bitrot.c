...
glusterd_bitrot_disable (glusterd_volinfo_t *volinfo, char **op_errstr)
{
        ...
        ret = dict_set_dynstr (volinfo->dict, VKEY_FEATURES_BITROT,
                               bitrot_status);
        if (ret) {
                gf_log (this->name, GF_LOG_ERROR, "dict set failed");
                goto out;
...
glusterd_manage_bitrot (int opcode)
{
        ...
        case GF_BITROT_OPTION_TYPE_DISABLE:
                ...
                /* TO DO:
                 * stop or start bitd service. once bitd daemon refactor patch
                 * merge or this patch become dependent of bitd daemon refactor
                 * patch remove below comments.
                 */
